...
cleanData=$(sed '1d' $inputFile | sed 's/^ *//g;s/ *$//;s/ \{1,\}/ /g' | cut -d ' ' -f1 --complement) 
...
populationNames=$(cut -d " " -f 3 <(echo -ne "$cleanData") | sort -u)
...
do
	...
	do
		...
		if [[ $(echo -ne $line | cut -d " " -f 3) == $populationName ]]
		then
...
plotCommands=$plotCommands"set title \"PCA graph\"\n"
...
do # vectors nohead lc $i
	...
	numberIndividuals=$[$(grep -c -w $populationName $inputFile)]
	plotCommands=$plotCommands" \""$outputFolder"TempFileForPopulation"$populationName".temp\" title \""$populationName" ("$numberIndividuals" individuals)\"," # \" using 1:2:(\$3-\$1):(\$4-\$2) with linespoints 
#title \""$populationName" ("$numberIndividuals" individuals)\"," # \"\" using 1:2 lt $i pt $i notitle, \"\" using 3:4 lt $i pt $i notitle," #notitle
